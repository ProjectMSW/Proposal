...
The bivariate and multivariate analyses employed to explore the relationship between total number of deaths, death rates, health, economic and population structure indicators are the scatterplot (`ggstatsplot` package), funnel plot (`ggplot2` package) and multiple linear regression (`olsrr` package). It was noted that the output plot of `ggscatterstats()` from `ggstatplot` was unable to be rendered interactive despite claims from the author that the package is compatible with `plotly`, a graphing library that creates interactive graphs. The output plots from the `olsrr` package are also not compatible with `plotly`.
