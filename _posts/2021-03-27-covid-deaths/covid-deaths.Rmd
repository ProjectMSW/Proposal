...
* *ggExtra*: for plotting of marginal distributions with ```ggMarginal()```
...
The next step would be to explore the packages to build the needed plots. The criteria to evaluate the plots are:
...
There are a few key arguments that are available in the function for customisation:
...
* `label.` arguments: set label parameters
...
The code chunk below generates a scatterplot of total deaths by hospital beds per thousand.

```{r}
sp_ggstatsplot_marginal <- 
  ggscatterstats(covid_deaths_imputed,
                 x = hospital_beds_per_thousand, # independent variable
                 y = total_deaths, # dependent variable
                 ...
                 ggplot.component = list(geom_smooth(method = "loess", size =0.05), # regression line
                                         scale_y_log10()), # change axes scale
...
The *car* package uses basic R graphics to generate the scatterplot. The key arguments available in the function for customisation are:
...
A ```scatterplot``` implementation of the plot from ```ggscatterstats()``` (as close as possible) is shown below.

```{r}
sp_car <- scatterplot(total_deaths ~ hospital_beds_per_thousand | continent, # group by continent
                      covid_deaths_imputed,
                      boxplots = "xy", # axis for marginal boxplot
                      regLine = list(method=lm, 
                                     lty=1, 
                                     lwd=1), # regression line to display and customisation
                      id = list(method="mahal", n=5), # label customisation
                      ellipse = list(levels=c(.5, .95), 
                                     robust=TRUE, 
                                     fill=TRUE, 
                                     fill.alpha=0.2), # ellipse customisation
                      smooth = list(smoother=loessLine,
                                    lty.var=2, 
                                    lwd.var=0.5), # smooth line to display and customisation
                      ...
                      log = "y", # change scale of y axis to log
...
Both packages allow for a wide variety of options for customisation, with *car::scatterplot* having an added feature of plotting ellipses. For *ggstatsplot::ggscatterstats*, most *ggplot* functions such as ```geom_smooth``` and ```scale_``` functions are supported, which makes it also fairly easy to implement for someone with basic knowledge of ggplot.

Although some flexibility is given to customise the plots, it requires much effort to make the changes with *car::scatterplot* due to the many "hidden" lists within the arguments. There are also some limitations to the arguments, such as the marginal distribution is limited to the boxplot and the display of data labels is limited to the values.

#### Clarity and aesthetic

The statistical test in the *ggstatsplot::ggscatterstats* scatterplot is informative to the more statistically inclined, with an option for the non-statistically inclined to turn off the statistical results. The graph is also visually appealing with the default settings for position, colour and data point opacity, thus little customisation to the aesthetics need to be done. 

On the other hand, the scatterplot from *car::scatterplot* is visually overwhelming as the ellipses, regression lines and smooth lines are plotted by groups, i.e. continent. When the group is defined, the plots by group cannot be turned off by setting the argument ```by.groups``` to ```FALSE```. In addition, the shape scale is automatically added which adds visual clutter to the plot. There is a need to explicitly amend ```pch``` to standardise the shape. The location of the marginal boxplot is near the axes labels at the bottom and left of plot, which does not give attention to the marginal boxplots.

In general, the scatterplot created using the *car* package is not as visually appealing than that from the *ggstatsplot* package. 
...
Another option is to build the scatterplot from scratch using ```ggplot()```, which can be passed into ```ggplotly()``` to make the plot interactive. 

```{r}
# base ggplot
sp_ggplot_base <- ggplot(covid_deaths_imputed, 
                    aes(x=hospital_beds_per_thousand, y=total_deaths, size=population)) +
...
However, an error message is encountered when rendering the plot with marginal distributions, added to the *ggplot* object with the use of ```ggMarginal()``` from the *ggExtra* package.
...
The arguments in the ```ggscatterstats()``` functions can mostly be manipulated at the first level, whereas the ```scatterplot()``` function utilises lists embedded in its arguments for more customisations e.g. ```smooth = list(smoother=loessLine,lty.var=2, lwd.var=0.5)```. This makes the former easier to code parameters to be passed into the function as arguments in the Shiny app. The same goes for the ```ggplot()``` function.
...
Two packages *FunnelPlotR* and *funnelR* will be explored to plot a funnel plot of case fatality rate (defined as the total number of deaths divided by the total number of cases) against total number of cases. Other rates can possibly be plotted using the funnel plot.

### Use *FunnelPlotR::funnel_plot*

The *FunnelPlotR* package uses *ggplot* to generate funnel plots. It requires a numerator (events of interest), denominator (population to be considered) and group. The key arguments available in the function for customisation are:
...
* ```group``` in this function is different from the scatterplot. Here, it defines the basic unit to be plotted. 
* There are no arguments available to customise the funnel plot using *ggplot* functions

```{r}
fp_funnelplotR <- funnel_plot(numerator = covid_deaths_imputed$total_deaths,
...
As the output is a "funnelplot" object, it cannot be passed to ggplotly to render the plot interactive so as to allow user to take a closer look at the plot area with high concentration of data points. An alternative will be to express the total deaths and cases in thousands so as to reduce the x-axis range.

```{r}
covid_deaths_fp <- covid_deaths_imputed %>%
  mutate(total_deaths=total_deaths/100000,
         total_cases=total_cases/100000)

fp_funnelplotR_div <- funnel_plot(numerator = covid_deaths_fp$total_deaths,
                              denominator = covid_deaths_fp$total_cases,
                              group = covid_deaths_fp$location,
                              data_type = "PR", # to plot proportions
                              limit = 99,
                              label_outliers = TRUE,
                              Poisson_limits = TRUE,
                              title = "Case Fatality Rate by Total Number of COVID-19 Cases",
                              x_label = "Total Number of COVID-19 Cases (hundred thousands)",
                              y_label = "Case Fatality Rate",
                              xrange = c(0, 330),
                              yrange = c(0, 0.25)
                              )
...
We can also change `xrange` to create the zoom-in effect, with the following code chunk.

```{r}
fp_funnelplotR_div_zoom <- funnel_plot(numerator = covid_deaths_fp$total_deaths,
                              denominator = covid_deaths_fp$total_cases,
                              group = covid_deaths_fp$location,
                              data_type = "PR", # to plot proportions
                              limit = 99,
...
The code chunk below shows an attempt to plot a funnel plot with the *funnelR* package. We will use the same dataset covid_deaths_fp to prevent runtime errors.

```{r}
covid_deaths_fp_fR <- covid_deaths_fp %>% 
  rename(n = total_deaths, d = total_cases)

funnelR_limits <- fundata(input = covid_deaths_fp_fR,
                          alpha = 0.95,
                          alpha2 = 0.998,
                          method = 'approximate', # exact or approximate: control limits smoothing 
                          step = 1
                          )

fp_funnelR <- funplot(covid_deaths_fp_fR, funnelR_limits, "continent")

fp_funnelR
```

There are no arguments within the function to customise the graph e.g. axes range and labels, opacity and size of points, title and legend. As ```funplot()``` returns a ggplot object, the funnel plot can be passed into ```ggplotly``` to render an interactive graphic.

```{r}
fp_funnelR_ggplotly <- ggplotly(fp_funnelR)

fp_funnelR_ggplotly
```

However, the interactivity is also limited to the functionality of base ggplot object.

### Drawing from scratch using *ggplot*

An alternative is to draw the funnel plot using *ggplot*, with the code chunk below.

```{r}
rate <- covid_deaths_fp$case_fatality_rate
number <- covid_deaths_fp$total_cases
...
# Draw funnel plot
fp_ggplot <- ggplot(covid_deaths_fp, 
                    aes(x = total_cases, y = case_fatality_rate)) +
  geom_point(aes(colour=continent, 
                 size=population, 
                 label=location), # to add labels into interactive plot
             alpha=0.4) +
  ...
  geom_hline(data = dfCI, aes(yintercept = rate.fem), 
             size = 0.4, colour = "grey40") +
  annotate("text", x = 0, y = -0.05, label = "95%", size = 3, colour = "grey40") +
  annotate("text", x = 20, y = -0.15, label = "99%", size = 3, colour = "grey40") +
  xlab("Total Number of COVID-19 Cases") + 
  ylab("Case Fatality Rate") 

fp_ggplot
```

The graph can also be made interactive with the following code chunk:
...
On the other hand, drawing from scratch with *ggplot* is also relatively easy once the control limits are calculated and offers the most customisation.
...
#### Interactivity
*FunnelPlotR* does not return a ggplot object, whereas both *funnelR* and *ggplot* returns a ggplot object, as such, the funnel plots from *funnelR* and *ggplot* can be made interactive after passing the ggplot object through ```ggplotly()```.

Generally, the arguments in all three functions are straightforward to use and there are limited parameters, other than the dependent and independent variables, in *FunnelPlotR* and *funnelR* that can be coded and passed as arguments into the functions.

#### Conclusion

Although *FunnelPlotR* is relatively easy to use, the function does not allow for more customisation due to its limitation of only taking in vectors for the key variables. In addition, it cannot be rendered into an interactive graphic, and the earlier limitation mentioned does not allow a workaround solution to use *FunnelPlotR*. As such, ```ggplot()``` is chosen to create the funnel plot in the final product.
