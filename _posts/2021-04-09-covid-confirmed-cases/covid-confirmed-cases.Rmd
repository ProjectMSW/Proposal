...
# Overview
The purpose of this assignment is to provides estimates of COVID-19 near-future case load in each country, based on global data collated by Johns Hopkins University. Generally, the data is a sequence of observations stored in time orders. 

Time series analysis attempt to analyze the time series data to extract meaningful characteristics of data and generate useful insights. For our case, we will attempt to forecast the near-future number of confirmed cases. Time series may have several characteristics that make their analysis different from other types of data.

* The time series variable may have a trend over time.
* The variable may exhibit cyclicity or seasonality. 
* The data may have serial correlation between subsequent observations.
...
These characteristics will affect the outcome of the forecasting models. Decomposing a time series into its components (trend, cycle, seasonality, and random (stationary)) is an important part of time series analysis because this is the primary assumption to forecast the time series in the future. This is generally done to help understand the time series better, but it can also be used to improve forecasts.

Therefore, we should allow users to visualise the underlying "patterns" of the time series data.
...
packages = c('xgboost','tidymodels','modeltime')
...
* confirmed_cases_country_level to store cumulative confirmed cases for the country (aggregating the province/state, city level data to Country/Region)
...
## Doing forecast with the Test Data

Fitting the forecast with the Test Data and plot the visualisation.
```{r}
UScalibration_tbl %>%
    modeltime_forecast(
...
To re-calibrate the model using the full dataset and forecast the next 10 days of confirmed cases.
...
## Using Shorter Time Period for the Forecast.

Observed that the US dataset have a spike of confirmed cases from Nov 2020 to Feb 2021. This "anomaly" might affect the accuracy of the forecast. Explore using only data from May 2020 to Oct 2020 to see if the forecast result improves.
...
From the result, it suggested that the "anomaly" in the dataset do affect the forecasted result. For example, the Prophet result is 33675.32 (RMSE) for Jul to Oct 2020 dataset and result is 241226.03 (RMSE) for the full set of US data. To improve the accuracy of the models, the application should allow the users to choose the period to be used for the forecasting.

# Explore other parameters within the Models.
...
```
Changing seasonality to Daily and change it to Multiplicative model.
...
refit_tbl <- calibration_tbl %>%
    modeltime_refit(data = chosendata)

refit_tbl %>%
    modeltime_forecast(h = "10 days", actual_data = chosendata) %>%
    plot_modeltime_forecast(
      .legend_max_width = 25, # For mobile screens
      .interactive      = TRUE,
      .title = "US"
